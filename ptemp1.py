...
class PurchaseSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        instance = self.context.get("purchase")
        ...
        if not no_validate_funds:
            default_currency_balance = CurrencyBalance.objects.filter(
                state=True,
                account=self.context["account"],
                currency=Currency.objects.get_default_currency(),
            ).first()
            if (
                not default_currency_balance
                or (not instance and (data["total"] > default_currency_balance.balance))
                or (
                    instance
                    and (
                        data["total"]
                        > default_currency_balance.balance + instance.total
                    )
                )
            ):
...
class PurchaseCalculateSerializer(PurchaseSerializer):
    class Meta:
        model = Purchase
    ...
    def calculate_is_suspicious_rate(self):
        n = 50
        last_n_purchases = Purchase.objects.filter(
            state=True,
            account=self.context["account"],
            currency=self.validated_data["currency"],
        )[:n]
        n = len(last_n_purchases)
        if n == 0:
            return False
        elif n == 1:
            mean = last_n_purchases[0].rate
            standard_deviation = 1
        else:
            total_weight = sum([(purchase.amount) for purchase in last_n_purchases])
            total_weight = total_weight if total_weight > 0 else 1
            mean = sum(p.rate * p.amount for p in last_n_purchases) / total_weight
            variance = sum(
                p.amount * ((p.rate - mean) ** 2) for p in last_n_purchases
            ) / (total_weight)
    ...
    def calculate(self):
        ...
        if not default_currency_balance or total > default_currency_balance.balance:
...
class UserPurchaseCalculateSerializer(PurchaseCalculateSerializer):
    class Meta:
        model = Purchase
